...
async function main() {
  ...
  console.log("Seed : distributeurs et pompes...");
  const codesPompes: { code: string; produit: "ESSENCE" | "GASOIL" }[] = [
    "S1-A", "G1-A", "S1-B", "G1-B",
    "S2-A", "G2-A", "S2-B", "G2-B",
    "S3-A", "G3-A", "S3-B", "G3-B",
    "S4-A", "G4-A", "S4-B", "G4-B",
    ...
    "G8-A", "G8-B", "G8-C", "G8-D",
    "G9-A", "G9-B", "G9-C", "G9-D",
  ].map((code) => ({ code, produit: code.startsWith("S") ? ("ESSENCE" as const) : ("GASOIL" as const) }));

  // Regroupées par distributeur (îlot physique), déduit du numéro dans le code — cf.
  // regrouperPompesParDistributeur dans stations.service.ts (même règle, dupliquée ici pour
  // ne pas dépendre du contexte Nest depuis ce script de seed autonome).
  const groupes = new Map<string, typeof codesPompes>();
  for (const p of codesPompes) {
    const numero = p.code.match(/^[A-Za-z]+(\d+)-/)?.[1]!;
    const nomDistributeur = `Distributeur ${numero}`;
    const liste = groupes.get(nomDistributeur) ?? [];
    liste.push(p);
    groupes.set(nomDistributeur, liste);
  }
  for (const [nomDistributeur, pompes] of groupes) {
    const distributeur = await prisma.distributeur.upsert({
      where: { stationId_nom: { stationId: pk8.id, nom: nomDistributeur } },
      update: {},
      create: { stationId: pk8.id, nom: nomDistributeur },
    });
    for (const p of pompes) {
      await prisma.pump.upsert({
        where: { stationId_code: { stationId: pk8.id, code: p.code } },
        update: {},
        create: { stationId: pk8.id, code: p.code, produit: p.produit, distributeurId: distributeur.id },
      });
    }
  }
